...
//creating .csv file with header--------------------
  ...
  $result_assign_brand1 = mysqli_query($con, "SELECT `category_code`,`brandname` FROM `assign_brands` WHERE `jrp_account_no`='$jrp_account_no' AND `col_1`='1'");
  ...
  WHERE `brand` = '$brandname' AND `brand` NOT IN (SELECT `brand` FROM `filter_brands`) AND `prod` = '$category_code' GROUP BY `jrpsku`;";
