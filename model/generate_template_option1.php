...
{              
  ...
  $report_indexes_id = $row_template_name['report_indexes_id'];
              ...
              //===================== template name
              $result_template = mysqli_query($con, "SELECT `brand_name`,`product_code_id`,`warehouse` FROM `template` WHERE `flag`='1' AND `brand_name` NOT IN (SELECT `brand` FROM `filter_brands`) AND `report_indexes_id`='$report_indexes_id' GROUP BY `brand_name`; ");
