...
{              
              ...
              //===================== template name
              $result_template = mysqli_query($con, "SELECT `brand_name`,`product_code_id`,`warehouse` FROM `template` WHERE `flag`='1' 
                                              AND `brand_name` NOT IN (SELECT `brand` FROM `filter_brands`) AND `report_indexes_id`='$report_indexes_id' group by `brand_name`; ");
              ...
              //===================== product_code
              $result_product_code = mysqli_query($con, "SELECT `category_code`, `product_desc` FROM `product_code` WHERE `product_code_id`='$product_code_id' AND `flag`='0'");
              while($row_product_code = mysqli_fetch_array($result_product_code)){
