...
$result_template_name = mysqli_query($con, "SELECT `index_name`, `report_indexes_id` FROM `report_indexes`");
  ...
  $report_indexes_id = $row_template_name['report_indexes_id'];
              ...
              {             
                            ...
                            $result_stock = mysqli_query($con, "SELECT SUM(`onhand`) as onhand,SUM(`commited`) as commited, `brand`,`jrpsku`,`description`,`cost`,`map`,`prod`,`col_1` 
                                                                FROM `stock` WHERE `brand`= '$brand_name' AND `prod`='$category_code' AND `col_1`='$warehouse' GROUP BY `jrpsku`;");
