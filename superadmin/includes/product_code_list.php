...
?>
    <input type="checkbox" onclick="toggle(this);" />Check all?<br />  
        <?php
            $result = mysqli_query($con, "SELECT `product_desc`,`category_code` FROM `product_code` WHERE `category_code` IN (SELECT `prod` FROM `stock` WHERE `brand` LIKE '%$from%') AND `flag`='0' GROUP BY `category_code`");  
            $count_name1 = 0;              
