<section class="content">
    ...
    <form role="form" action="../model/assign_brands_group.php" >
        <div class="card-body">
            ...
            <?php 
            // Fetch Department
            //SELECT * FROM `product_code` WHERE `product_desc` REGEXP BINARY '^[A-Z]+$'; 
            $sql_department = "SELECT * FROM `product_code` WHERE `category_code` != '' AND `product_desc` NOT REGEXP BINARY '^[A-Z]+$'";
            $department_data = mysqli_query($con,$sql_department);
            while($row = mysqli_fetch_assoc($department_data) ){
                $departid = $row['category_code'];
                $depart_name = $row['product_desc'];
...
<div>Brands </div>
<select id="sel_user"><option value="0">- Select -</option></select>

<!-- <input type="checkbox" id="sel_user" value="0" onclick="toggle(this);" />Check all?  -->
